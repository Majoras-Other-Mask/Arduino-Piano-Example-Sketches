...
ISR(TIMER2_COMPA_vect) {
  ...
  phaseDelta = pitch;                           // this is just pitch (although not in Hz)

  phaseAccum = phaseAccum + phaseDelta; // + modulatorSigned;
  index = phaseAccum >> 8;
  // get thie current step for the carrier from the wavetable too
  memcpy_P(&oscillatorOut,&waveTable[waveForm*256+index],1);

  // output gain
  sample = oscillatorOut * gain;
