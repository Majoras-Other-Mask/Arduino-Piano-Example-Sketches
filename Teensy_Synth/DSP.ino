/*********************************************
 * Plays Sine Tone on Arduino Pocket Piano
 * Author: Owen Osborn, Copyright: GPL 
 * 
 *  This uses Direct Digital Synthesis (DDS)  for tone generation
 * 
 *   
 * 5 October 2007
 *
 * FOR MCP4921 DAC
 *
 * Chip type           : ATMEGA168
 * Clock frequency     : 16MHz
 *********************************************/

#define SPI_SCK 5
#define SPI_MOSI 3
int m; //counter

// Here are variables for digital audio.  
// They are declared using normal AVR-GCC notation ('uint8_t' for unsigned 8 bit number)

// Here is the sine wave and triangle wave.  It is a one cycle of sine tone, 255 8 bit samples, in hex values
// you could put any wave form you line in here...
uint8_t sawTable[]={
  };

uint8_t sineTable[]={
  };

uint8_t triTable[]={
  };

uint8_t squTable[]={
  };

uint8_t pulseTable[]={
  };

uint8_t fuzzTable[]={
  };

uint8_t digi1Table[]={
  };

uint8_t tanTable[]={
  };


// final sample that goes tote DAC
uint16_t sample;

//// variables for carrier oscillator
//uint16_t phaseAccumCarrier[] = {0, 0, 0, 0};
//uint16_t phaseDeltaCarrier[] = {0, 0, 0, 0};
//uint8_t indexCarrier[] = {0, 0, 0, 0};
//uint8_t carrier[] = {0, 0, 0, 0};
//uint8_t carrierAmp[] = {0, 0, 0, 0};
//
//// variables for frequency mod oscillator
//uint16_t phaseAccumModulator[] = {0, 0, 0, 0};
//uint16_t phaseDeltaModulator[] = {0, 0, 0, 0};
//uint8_t indexModulator[] = {0, 0, 0, 0};
//int16_t modulatorSigned[] = {0, 0, 0, 0};
//uint16_t modulator[] = {0, 0, 0, 0};

// variables for carrier oscillator
uint16_t phaseAccumCarrier; //[] = {0, 0, 0, 0};
uint16_t phaseDeltaCarrier; //[] = {0, 0, 0, 0};
uint8_t indexCarrier; //[] = {0, 0, 0, 0};
uint16_t carrier; //[] = {0, 0, 0, 0};
uint8_t carrierAmp; //[] = {0, 0, 0, 0};

// variables for frequency mod oscillator
uint16_t phaseAccumModulator; //[] = {0, 0, 0, 0};
uint16_t phaseDeltaModulator; //[] = {0, 0, 0, 0};
uint8_t indexModulator; //[] = {0, 0, 0, 0};
int16_t modulatorSigned; //[] = {0, 0, 0, 0};
uint16_t modulator; //[] = {0, 0, 0, 0};


// the two bytes that go to the DAC over SPI
uint8_t dacSPI0;
uint8_t dacSPI1;

// variables for oscillators
uint16_t accumulator[] = {0, 0, 0, 0};  // large number holds phase
uint8_t ind[] = {0, 0, 0, 0};   // index for wave lookup (the upper 8 bits of the accumulator)
uint16_t oscil[] = {0, 0, 0, 0};   // oscillator output
   

// timer 2 is audio interrupt timer
void sendNote() {
  if (mode == 0 || mode == 1) { 
    // calculate frequency mod
    for (m=0; m<4; m++) { 
      accumulator[m] = accumulator[m] + freq[m];
      Serial.println(accumulator[0]); 
      ind[m] = accumulator[m] >> 8;
      oscil[m] = waveFormFunc(ind[m], waveForm); 
    }
//    sample = (oscil[0] + oscil[1] + oscil[2] + oscil[3]) << 6;
     
    sample = (oscil[0] + oscil[1] + oscil[2] + oscil[3]); //uncomment when using internal DAC

  
  } else if (mode == 2) { 

    phaseDeltaModulator = (freq[0] * knob1) >> 6;   // calculate modulator frequency for a  given harmonicity
    phaseDeltaCarrier = freq[0];                           // this is just pitch (although not in Hz)
    // calculate frequency mod
    phaseAccumModulator = phaseAccumModulator + phaseDeltaModulator; 
    indexModulator = phaseAccumModulator >> 8;
    modulator = waveFormFunc(indexModulator, waveForm);
    modulator = (modulator * knob2) >> 3;
    modulatorSigned = modulator - ((128 * knob2) >> 3);   // center at 0
  
    // get carrier frequency
    phaseDeltaCarrier += modulatorSigned;
  
    // calculate carrier
    phaseAccumCarrier = phaseAccumCarrier+ (phaseDeltaCarrier);
    indexCarrier = phaseAccumCarrier >> 8;
    carrier = waveFormFunc(indexCarrier, waveForm);
    // output gain
    sample = (carrier) * gn; //
   
  }

  analogWrite(A22, sample); //uncomment when using internal DAC
  
//  // format sample for SPI port
//  dacSPI0 = sample >> 8;
//  dacSPI0 >>= 4;
//  dacSPI0 |= 0x30;
//  dacSPI1 = sample >> 4;
//
//  SPI.beginTransaction(SPISettings(4000000, MSBFIRST, SPI_MODE0));
//  digitalWriteFast(slaveSelectPin, LOW);
//  SPI.transfer(dacSPI0);
//  SPI.transfer(dacSPI1);
//  digitalWriteFast(slaveSelectPin, HIGH);
//  SPI.endTransaction();
}
