...
uint8_t index1 = 0;        // index for wave lookup (the upper 8 bits of the accumulator)
...
void setup(){
  ...
  myBtnReader.begin(checkSwitch, 64);
...
void checkSwitch(){
  ...
  index1 = accumulator >> 8;   // use top 8 bits as wavetable index

  oscil = sineTable[index1];    // get sample from wave table
